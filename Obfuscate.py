...
def main():
    ...
    model = Model(len(features), args.authors_total)
    ...
    for data, label in ig_set:
        ...
        all_attributions.extend(attributions.tolist())

    data['attribution'] = all_attributions

    isValid = lambda index: index >= num_char and index < num_char + num_pos
